...
#define CENTER_OFFSET (CANVAS_3D_W>>1)
...
/*=========================*/
LIB_API void
V3D_Flip(ui16 p_nOffsetX, ui16 p_nOffsetY)
{
	ui16	*p1;
	ui16	*p2	= m_dest;
	i32		i	= SCH;
...
///////////
  curV= mesh->m_vb.m_start=(*p_vertex);
  curT= mesh->m_tb.m_start=(*p_texCoord);
  curI= mesh->m_ib.m_start=(*p_index);

  mesh->m_vb.m_nb=  md2_hdr.numVertices;
  mesh->m_ib.m_nb=  md2_hdr.numTriangles;
  mesh->m_tb.m_nb=  md2_hdr.numTexCoords;
  mesh->m_nNbFrame= md2_hdr.numFrames;
  mesh->m_cptFrame= 0;
//////////
...
#define FPP_DIV (1<<FPP)
#define FPMUL_HUGE(x,y) ( x*(y>>FPP) + ((x*(y%FPP_DIV) )>>FPP ))
...
V3D_ProcessVB(MATRIX* p_mat, VB* p_vb)
{
  ...
  for (ui32 i = 0; i < p_nb;i++){
	...
	*/
	/*
	p_tri[i].rot[0]=	FPMUL_HUGE(p_mat->l[0],	ox) +
    					FPMUL_HUGE(p_mat->l[1],	oy) +
    					FPMUL_HUGE(p_mat->l[2],	oz) + p_mat->l[3];
    p_tri[i].rot[1]=	FPMUL_HUGE(p_mat->l[4],	ox) +
    					FPMUL_HUGE(p_mat->l[5],	oy) +
    					FPMUL_HUGE(p_mat->l[6],	oz) + p_mat->l[7];
    p_tri[i].rot[2]=	FPMUL_HUGE(p_mat->l[8],	ox) +
    					FPMUL_HUGE(p_mat->l[9],	oy) +
    					FPMUL_HUGE(p_mat->l[10],	oz) + p_mat->l[11];
	*/

	p_tri[i].rot[0]=	((p_mat->l[0 ])*(ox>>FPP)) +
    					((p_mat->l[1 ])*(oy>>FPP)) +
    					((p_mat->l[2 ])*(oz>>FPP)) + p_mat->l[3];
    p_tri[i].rot[1]=	((p_mat->l[4 ])*(ox>>FPP)) +
    					((p_mat->l[5 ])*(oy>>FPP)) +
    					((p_mat->l[6 ])*(oz>>FPP)) + p_mat->l[7];
    p_tri[i].rot[2]=	((p_mat->l[8 ])*(ox>>FPP)) +
    					((p_mat->l[9 ])*(oy>>FPP)) +
    					((p_mat->l[10])*(oz>>FPP)) + p_mat->l[11];



    //p_tri[i].sX= IFP(CENTER_OFFSET) + FPDIV(p_tri[i].rot[0], p_tri[i].rot[2] >> 9);
	//p_tri[i].sY= IFP(CENTER_OFFSET) + FPDIV(p_tri[i].rot[1], p_tri[i].rot[2] >> 9);

	/*->Krishol*/
	if (p_tri[i].rot[2] >> 9) //Krishol
	{
		//p_tri[i].sX= IFP(CENTER_OFFSET) + FPDIV(p_tri[i].rot[0], p_tri[i].rot[2] >> 9);
		//p_tri[i].sY= IFP(CENTER_OFFSET) + FPDIV(p_tri[i].rot[1], p_tri[i].rot[2] >> 9);
		p_tri[i].sX= IFP(CENTER_OFFSET) + ((p_tri[i].rot[0] / (p_tri[i].rot[2]>>9)) << 10);
		p_tri[i].sY= IFP(CENTER_OFFSET) + ((p_tri[i].rot[1] / (p_tri[i].rot[2]>>9)) << 10);
	}
	else
	{
		p_tri[i].sX= IFP(CENTER_OFFSET);
		p_tri[i].sY= IFP(CENTER_OFFSET);
	}
	/*<-Krishol*/
...
*/
  ...
  , 		oy
  , 		oz;
  static i32 m_cptFrame= 0;
  VX2*	p_tri	= p_mesh->m_vb.m_start;
  i32	p_nb	= p_mesh->m_vb.m_nb;
  VX2*	p_tri2	= p_tri+(p_nb* /*p_mesh->*/ m_cptFrame);
  ...
  if((++/*p_mesh->*/m_cptFrame)>=p_mesh->m_nNbFrame)
    /*p_mesh->*/m_cptFrame=0;
}
/*=========================*/
LIB_API void
...
{
//  i32 	drXMin=CANVAS_3D_W, drXMax= 0
//  , 	drYMin= CANVAS_3D_H, drYMax=0;
  ui32 	i;
  i32	j, k, index
  ,	maxy, miny
//  ,	sxI, syI
  , sx1, sy1, sx2, sy2, z1, z2
...
//			(p_tri->m_B));
    ...
    for ( j = miny; j < maxy; j++ ){
      ...
      dst += CANVAS_3D_W; //Krishol : 1200
      zbf += CANVAS_3D_W; //Krishol : 1200
      m_Left[j] = IFP(CANVAS_3D_W); //Krishol : 1200
      m_Right[j] = 0;
    }
  }
}
/*=========================*/
/*
FPP : fix point precision
u = x coord pour une texture.
v = y coord pour une texture.
*/
...
V3D_ID_Text(VX* p_tri, TEXTURE* p_tex, ui32 p_nb)
{
	...
	//sx1-2 et sy1-2 sont dejas utiliser pour comparer les pts...
	static i32		sx4,  sy4, sx5,  sy5, sx6,  sy6; //traingle validation
	...
	i32		j,
			...
			maxy, miny, //loop
			sx1,  sy1,  //triangle coord 1.
			sx2,  sy2,  //triangle coord 2.

			z1,   z2,

			fix,				// ??? fix = FP_ONE - (sy1 & (FP_ONE - 1));
			d,					// sy2 - sy1;
			du, dv, dz, dx,		// distance entre les pts (ex : u2 - u1)
			cu, cv, cz, cx, cy,

			y1, y2,				//y1, y2, loop : for (k = y1; k < y2; k++)
			x1, x2,				//test if (x2 > x1) x1 = m_Left [j] >> FPP; x2 = m_Right[j] >> FPP;
			u1, v1,				//texture x coord.
			u2, v2;				//texture y coord.



	ulong	cuv,				//cuv : ??? offset d'ecriture
			duv;				//duv : ??? offset : increment pour cuv
	ui16	cz2;				//cz2 : ??? offset d'ecriture + test d'ecriture
	...
	for (i = 0; i < p_nb;	i+= 3, p_tri+= 3)
	{
		...
		sx6 = p_tri[2].sX >> (FPP - 6);
		sy6 = p_tri[2].sY >> (FPP - 6);
		//test if the triangle texture have to be drawn.
		if((((sx4 - sx5) * (sy6 - sy5)) - ((sy4 - sy5) * (sx6 - sx5))) <= 0)
			continue;

		mustClip= (p_tri[0].sX < 0 || p_tri[0].sY > (CANVAS_3D_W<<(FPP)) ||
				   p_tri[1].sX < 0 || p_tri[1].sY > (CANVAS_3D_W<<(FPP)) ||
				   p_tri[2].sX < 0 || p_tri[2].sY > (CANVAS_3D_W<<(FPP)) );
		...
		//Assert triangle coord 1
		if(sx1 <= 0)
			sx1=0;
		if(sx1 >= IFP(CANVAS_3D_W))
			sx1=IFP( (CANVAS_3D_W-1) );
		if(sy1 <= 0)
			sy1=0;
		if(sy1 >= IFP(CANVAS_3D_W))
			sy1=IFP( (CANVAS_3D_W-1) );

		//Get texture coord1
		u1 = p_tri[0].m_U1;
		v1 = p_tri[0].m_V1;

		//Z coord, 0x7FFFFFF0 for invese the value
		/*->Krishol*/
		if (p_tri[0].rot[2])
			z1 = (0x7FFFFFF0 / p_tri[0].rot[2]) << FPP;
		else
		...
		for (j = 0; j < 3; j++ )
		{
			//j=0/index=1		j=1/index=2		j=2/index=0
			index= ((j+1)>2)?0:j+1;

			//Get texture coordX (index)
			sx2 = p_tri[index].sX;
			sy2 = p_tri[index].sY;

			//Assert triangle coordX
			if(sx1 <= 0)
				sx1=0;
			if(sx1 >= IFP(CANVAS_3D_W))
				sx1=IFP( (CANVAS_3D_W-1) );	//Krishol : 1200
			if(sy1 <= 0)
				sy1=0;
			if(sy1 >= IFP(CANVAS_3D_W))
				sy1=IFP( (CANVAS_3D_W-1) );	//Krishol : 1200
			...
			/*************************************/
			//compare y1 triasngle with compare yX triasngle
			if (sy2 > sy1)
			{
				//Texture coord
				cx = sx1;
				...
				//Distance entre sy1 et sy2
				d = sy2 - sy1;

				//Si la distance est plus grande que 4
				if ((d > 4) || (d < -4))
				{
					//Calcule la distance entre le coordonne du pt1 et pt2 du triangle
					dx = FPDIV( sx2 - sx1, d );
					du = FPDIV( u2 - u1, d );
					dv = FPDIV( v2 - v1, d );
					dz = FPDIV( z2 - z1, d );
				...
				//Fait l'interpolation lineaire et pour l'affichage du triangle et pour aller chercher ca texture
				fix = FP_ONE - (sy1 & (FP_ONE - 1));

				//Cumule le fix pour, cx, cu, cv, cz
				cx += FPMUL( fix, dx );
				cu += FPMUL( fix, du );
				cv += FPMUL( fix, dv );
				cz += FPMUL( fix, dz ); //Krishol ??? Demander une confirmation a alain pour etre certain.
			}
			else
			{
				//Texture coord
			...
			/*->Krishol*
			if (y1 < 0           ) y1= 0;
			if (y1 >= CANVAS_3D_H) y1= CANVAS_3D_H-1;
			if (y2 < 0           ) y2= 0;
			if (y2 >= CANVAS_3D_H) y2= CANVAS_3D_H-1;
			/*<-Krishol*/

			for (k = y1; k < y2; k++)
			{
				//Krishol ???
				// crash si m_Left[k], m_ULeft[k], m_VLeft[k], m_ZLeft[k] sont en dehors de l'ecran...
				//utf

				/*
				if (y1 < 0           )
					break; //Krishol ??? Demander une confirmation a alain pour etre certain.
				if (y2 >= CANVAS_3D_H)
					break; //Krishol ??? Demander une confirmation a alain pour etre certain.
				*/
...
////////////////////////////////
		        ...
		        //m_ZLeft[k] = (cz<0)?0:cz;
				//et
				//m_Right[k] = (cx>=IFP( CANVAS_3D_W ))?IFP( CANVAS_3D_W-1 ):cx;
				//m_ZRight[k]= (cz>=IFP( CANVAS_3D_W ))?IFP( CANVAS_3D_W-1 ):cz;
/*
				 if(cx < 0)
				 {
					m_Uleft[k]= -cx*du
...
///////////////////////////////////////
/*
				if (m_Left [k] <				0  ) m_Left [k]= 0;
				if (m_Left [k] > IFP( CANVAS_3D_W )-1) m_Left [k]= IFP( CANVAS_3D_W )-1;
				if (m_Right[k] <				0  ) m_Right[k]= 0;
				if (m_Right[k] > IFP( CANVAS_3D_W )-1) m_Right[k]= IFP( CANVAS_3D_W )-1;

				if (m_ULeft [k] <				0  ) m_ULeft [k]= 0;
				if (m_ULeft [k] > IFP( CANVAS_3D_W)-1) m_ULeft [k]= IFP( CANVAS_3D_W )-1;
				if (m_URight[k] <				0  ) m_URight[k]= 0;
				if (m_URight[k] > IFP( CANVAS_3D_W)-1) m_URight[k]= IFP( CANVAS_3D_W )-1;

				if (m_ZLeft [k] <				0  ) m_ZLeft [k]= 0;
				if (m_ZLeft [k] > IFP( CANVAS_3D_W)-1) m_ZLeft [k]= IFP( CANVAS_3D_W )-1;
				if (m_ZRight[k] <				0  ) m_ZRight[k]= 0;
				if (m_ZRight[k] > IFP( CANVAS_3D_W)-1) m_ZRight[k]= IFP( CANVAS_3D_W )-1;
*/
/////////////////////////////////////////
			...
			sx1 = sx2;	//prepare next point to check
			sy1 = sy2;	//prepare next point to check
			u1 = u2;	//prepare next point to check
			v1 = v2;	//prepare next point to check
			z1 = z2;	//prepare next point to check
		}

		/*->Krishol*/
		//FIXME : quick clip,
		//Clip sur l'axe des y
		if (maxy > CANVAS_3D_H) maxy= CANVAS_3D_W;
		if (miny <           0) miny=           0;

		/*<-Krishol*/

		//Drawing Offset.
		ui16* dst = m_dest	  + (miny * CANVAS_3D_W);	//Destination
		ui16* zbf = m_ZBuffer + (miny * CANVAS_3D_W);	//ZBuffer
		...
		for ( j = miny; j < maxy; j++ )
		{
			...
			if (x2 > x1)
			{
				...
				for ( i32 k = x1; k < x2; k++ )
				{
					//Clip sur l'axe des x
					/*->Krishol*/
//					if(/*(ui16)*/(dst + k) > /*(ui16)*/(m_dest + (CANVAS_3D_W*viewPort)))
//					if(/*(ui16)*/(dst + k) < /*(ui16)*/(m_dest + (CANVAS_3D_W*(CANVAS_3D_W-viewPort))))
					if ( (k > viewPort) && (k < CANVAS_3D_W-viewPort) )
					/*<-Krishol*/
					{


						//if((dst + k) < (m_dest + (CANVAS_3D_W * CANVAS_3D_H))) FIXME
						if(*(zbf + k) < cz2)
						{
...
V3D_IDI_Flat(VX2* p_ver, TRI2* p_tri, TC2* p_texCoord, TEXTURE* p_tex, ui32 p_nbV, ui32 p_nbI)
{
  ...
  //todo incrementer ptri a place de [i]
    ...
    if((((sx4 - sx5) * (sy6 - sy5)) - ((sy4 - sy5) * (sx6 - sx5))) <= 0)
      continue;
    miny = CANVAS_3D_W; //Krishol : 1200
    maxy = 0;
    sx1 = p_ver[p_tri[i].idxVertex[0]].sX;
    sy1 = p_ver[p_tri[i].idxVertex[0]].sY;
    if(sx1 <= 0)		sx1=0;
    if(sx1 >= IFP(CANVAS_3D_W)) sx1=IFP( (CANVAS_3D_W-1) );	//Krishol : 1200
    if(sy1 <= 0)		sy1=0;
    if(sy1 >= IFP(CANVAS_3D_W))	sy1=IFP( (CANVAS_3D_W-1) );	//Krishol : 1200
    ...
    ui16* dst = m_dest + (miny * CANVAS_3D_W); //Krishol : 1200
    ui16* zbf = m_ZBuffer + (miny * CANVAS_3D_W); //Krishol : 1200
...
V3D_IDI_Text(VX2* p_ver, TRI2* p_tri, TC2* p_texCoord, TEXTURE* p_tex, ui32 p_nbV, ui32 p_nbI)
{
	...
	//todo incrementer ptri a place de [i]
		...
		//z1 = (0x7FFFFFF0 / p_ver[p_tri[i].idxVertex[0]].rot[2]) << FPP;
		/*->Krishol*/
		if (p_ver[p_tri[i].idxVertex[0]].rot[2])
			z1 = (0x7FFFFFF0 / p_ver[p_tri[i].idxVertex[0]].rot[2]) << FPP;
		else
			z1 = 0x7FFFFFF0;
		...
		for (j = 0; j < 3; j++ )
		{
			...
			/*->Krishol*/
			if (p_ver[p_tri[i].idxVertex[index]].rot[2])
				z2 = (0x7FFFFFF0 / p_ver[p_tri[i].idxVertex[index]].rot[2]) << FPP;
			else
				z2 = 0x7FFFFFF0;
